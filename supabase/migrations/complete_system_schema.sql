...
-- Patient consent lifecycle tracking
CREATE TABLE IF NOT EXISTS consent_records (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    patient_id UUID REFERENCES patients(id) ON DELETE SET NULL,
    consent_type VARCHAR(50) NOT NULL, -- 'data_sharing', 'terminology_upload', 'fhir_export'
    consent_status VARCHAR(20) DEFAULT 'granted', -- 'granted', 'withdrawn', 'expired'
...
-- Insert sample API clients (only if table has the expected structure)
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'api_clients' AND column_name = 'client_type'
    ) THEN
...
-- Insert sample consent records for compliance testing
INSERT INTO consent_records (patient_id, consent_type, consent_status, consent_version, consent_scope, recorded_by, recorded_by_role, metadata)
SELECT p.id,
       'fhir_export' AS consent_type,
       'granted' AS consent_status,
       'v1.0' AS consent_version,
       'Allow export of anonymised FHIR bundles to accredited research partners' AS consent_scope,
       'Dr. Priya Sharma' AS recorded_by,
       'Clinical Administrator' AS recorded_by_role,
       jsonb_build_object('reference_id', 'CONSENT-001', 'channel', 'digital') AS metadata
FROM patients p
WHERE p.medical_record_number = 'PAT001'
ON CONFLICT DO NOTHING;

INSERT INTO consent_records (patient_id, consent_type, consent_status, consent_version, consent_scope, effective_from, effective_until, recorded_by, recorded_by_role, metadata)
SELECT p.id,
       'data_sharing' AS consent_type,
       'withdrawn' AS consent_status,
       'v1.1' AS consent_version,
       'External sharing with non-accredited partners' AS consent_scope,
    NOW() - INTERVAL '90 days' AS effective_from,
    NOW() - INTERVAL '30 days' AS effective_until,
       'System Automation' AS recorded_by,
       'Compliance Engine' AS recorded_by_role,
       jsonb_build_object('reference_id', 'CONSENT-REVOKED-045', 'reason', 'Patient requested revocation via portal') AS metadata
FROM patients p
WHERE p.medical_record_number = 'PAT002'
ON CONFLICT DO NOTHING;
...
COMMENT ON TABLE consent_records IS 'Patient consent lifecycle tracking for ISO 22600 compliance';
